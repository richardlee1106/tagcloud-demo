...
OUTPUT_DIR = Path(__file__).parent / "experiment_results_v61"
...
class FullModelV61(nn.Module):
    ...
    def forward(self, poi_f, poi_c, adj, edge_f, block_f, batch_mapping=None):
        # 基础特征映射
        emb1 = self.cat_emb(poi_f[:, 0].long().clamp(0, self.schema.num_categories-1))
        emb2 = self.lu_emb(poi_f[:, 1].long().clamp(0, self.schema.num_landuse-1))
        emb3 = self.rd_emb(poi_f[:, 2].long().clamp(0, self.schema.num_road_class-1))
        emb4 = self.num_proj(poi_f[:, 3:6])
        emb5 = self.spatial_enc(poi_c)
        
        h = self.input_proj(torch.cat([emb1, emb2, emb3, emb4, emb5], dim=-1))
    ...
    def inference(self, all_f, all_c, global_adj, all_edge_f, block_f, chunk_size=100000):
        ...
        for i in range(0, N, chunk_size):
            ...
            e1 = self.cat_emb(fi[:, 0].long().clamp(0, self.schema.num_categories-1))
            ...
            xs.append(self.input_proj(torch.cat([e1, e2, e3, e4, e5], dim=-1)))
