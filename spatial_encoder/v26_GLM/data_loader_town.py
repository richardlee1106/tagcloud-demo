    ...
    def load_cells(
        ...
        for i, row in enumerate(rows):
            ...
            metadata.append({
                ...
                'poi_density': float(row['poi_density'] or 0),
                'category_entropy': float(row['category_entropy'] or 0),
                'dominant_category': row['dominant_category'],
                'category_distribution': cat_dist if isinstance(cat_dist, dict) else {},
                'road_count': row['road_count'],
                'road_length_km': round(row['road_length_km'] or 0, 2),
                'population_density': round(pop_density, 0),
                'landuse_mix': float(row['landuse_mix'] or 0),
                'dominant_landuse': row['dominant_landuse'],
