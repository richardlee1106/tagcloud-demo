# SpatialRAG-WebGIS DSL 与路由综合设计（整合版 v1.2）
...
- 基线文档:
  ...
  - `docs/2026-03-03-spatial-dsl-routing-integrated-v1.1.md`
- 本版变更（v1.2 相对 v1.1）:
  1. 修正 `context_binding` 时效判定逻辑（由"时间差阈值"改为"viewport_hash 变更检测"）
  2. 明确 `patch_ops` 遵循 RFC 6902 JSON Patch，声明支持的操作子集
  3. 新增 `prefetch_wasted` 可观测指标（Streaming 预取质量度量）
  4. 第10节耗时预算表补充"实测回填"要求说明
...
1. `context_binding`：用于 Spatial Context Binding 的一致性校验（见第5节）。
...
1. **主判断**：对比请求携带的 `context_binding.viewport_hash` 与服务端会话最后记录的 `viewport_hash`：
   - 若**一致**：无论 `captured_at_ms` 距请求发出多久，均视为有效，直接继续执行。
   - 若**不一致**（地图在用户打字期间发生了移动）：
     - 低风险场景：自动用新的 viewport 状态刷新 `scope.viewport` 后继续，标记 `context_refreshed=true`。
     - 高风险场景（`risk_level=high/critical`）：转 `clarification_needed`，由用户确认以哪个视角为准。
2. `captured_at_ms` 字段保留，仅用于审计日志与异步质量分析，不参与执行阻断决策。
...
> **v1.2 声明**：`patch_ops` 遵循 **RFC 6902 JSON Patch** 标准（参见 <https://datatracker.ietf.org/doc/html/rfc6902）。>
...
> **v1.2 说明**：下表中的数值为上线前的**目标预算**（工程估算），不代表实测基准。
...
| Streaming 预取重叠收益 | -120 | -350 | 与 planner 并发重叠（乐观估算，需实测验证） |
...
| **总计（参数通道）** | **~650** | **~1800** | **目标线（待实测回填）** |
| **总计（解释通道）** | **~980** | **~8200** | **包含 writer（待实测回填）** |
...
| P0 | Context Binding 一致性校验（viewport_hash 变更检测） | 高（避免错查） | 中 | 前后端协议 |
| P0 | 多轮 DSL patch 机制（RFC 6902） | 高（交互体验） | 中 | 会话存储 |
...
1. 实现 `context_binding` 采样与一致性检查（基于 `viewport_hash` 变更检测）。
...
4. **Phase C 完成后：将第10节耗时预算表用实测 P50/P95 回填，升级为实测基准。**
...
2. 在请求头或 options 中加入 `context_binding`，后端改为通过 `viewport_hash` 比对来判断上下文是否失效（不再用时间差）。
3. 在 Planner 输出中新增 `revision` 元信息，先支持 `rebuild` 和一个 `replace` patch（遵循 RFC 6902）。
4. 在 telemetry 打点:
...
2. v1.2 在 v1.1 基础上进一步修正了三处具体工程缺陷：
   - **Context Binding**：用 `viewport_hash` 变更检测取代"时间差阈值"，避免误打断正常用户。
   - **patch_ops 格式**：明确遵循 RFC 6902，约定支持的操作子集，消除开发歧义。
   - **Prefetch 可观测性**：新增 `prefetch_wasted` 指标，让预取质量形成反馈闭环。
3. 按本整合版推进，可同时守住:
   - 正确性（前置约束）
   - 时延（并发重叠）
