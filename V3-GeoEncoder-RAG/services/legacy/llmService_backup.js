 ...
 * 默认配置：qwen3.5-2b @ http://127.0.0.1:11434
 *
 * Author: Sisyphus
 * Date: 2026-03-21
 */
...
export async function getLLMConfig() {
  if (LLM_CONFIG.useOllama) {
    ...
    if (available) {
      console.log('[LLM] Using Ollama:', LLM_CONFIG.ollama.model);
      return {
        baseUrl: LLM_CONFIG.ollama.baseUrl,
  ...
  try {
    ...
    if (response.ok) {
      console.log('[LLM] Using LM Studio:', LLM_CONFIG.lmstudio.model);
      return {
        baseUrl: LLM_CONFIG.lmstudio.baseUrl,
        model: LLM_CONFIG.lmstudio.model,
  ...
  if (LLM_CONFIG.remote.baseUrl) {
    console.log('[LLM] Using remote API:', LLM_CONFIG.remote.model);
    return {
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          // 如果有完整的思考标签，删除它们
          const startTag = String.fromCharCode(60, 116, 104, 105, 110, 107, 62);
          const endTag = String.fromCharCode(60, 47, 116, 104, 105, 110, 107, 62);

          if (accumulated.includes(startTag) && accumulated.includes(endTag)) {
            // 完整的思考标签，删除
            clean = accumulated.replace(/ halls [sS]*? halls /g, '');
          } else if (accumulated.includes(startTag)) {
            // 只有开始标签，还在思考中
            continue;
          }

          // 只输出新增部分
          console.log(`[LLM Stream] clean.length=${clean.length}, outputLen=${outputLen}, accumulated=${accumulated.slice(0, 50)}`);
