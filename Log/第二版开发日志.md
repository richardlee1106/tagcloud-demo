...
2.  **Map -> TagCloud**：
    ...
    *   ****ʹ 50ms  (`debounce`) ٸƵͨ `emit('hover-feature')` ݴݸ `hoveredFeatureId`
...
2.  **Ż**ʹԶ `throttle` 500ms  `map-move-end` ¼ķƵʣֹء
3.  **数据过滤**：
...
1.  ****ԭƳ `drawInteraction` õĽöʧ򱻸ǣͼڲĽȻڣ¡޷ֹͣ
...
2.  **㼶** `.top-controls`  `z-index: 2000`  `position: relative`˵ͼڵ⡣
3.  **悬浮控件**：将实时过滤开关封装在 `.map-filter-control` 中，使用 `position: absolute; top: 10px; right: 10px;` 悬浮于地图右上角。
4.  ****ʹ Flexbox  `justify-content: space-between` ( div ֱ flex-start/flex-end) ʵ󡢰ťҵĲ֡
...
*   ****һãͨ `map.getInteractions()` ȡн (`instanceof Draw`) Ƴȷ
